...
{
	{
		...
		// DONE(student): Add or change the object colors
...
void Lab8::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, const glm::vec3& color)
{
	...
	// DONE(student): Send spotlight details
	int isSpotlightLocation = glGetUniformLocation(shader->program, "isSpotlight");
	glUniform1i(isSpotlightLocation, (int)isSpotlight);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		// DONE(student): Set any other keys that you might need
		if (window->KeyHold(GLFW_KEY_1))
		{
			angle -= deltaTime * RADIANS(30);
		}

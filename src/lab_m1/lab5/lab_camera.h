...
namespace implemented
{
	...
	public:
		...
		Lab5Camera(const glm::vec3& position, const glm::vec3& center, const glm::vec3& up)
		{
		...
		{
			// DONE(student): Translate the camera using the `forward` vector.
		...
		{
			// DONE(student): Translate the camera using the `up` vector.
			position += glm::normalize(up) * distance;
		}

		void TranslateRight(float distance)
		{
			// DONE(student): See instructions below. Read the entire thing!
		...
		{
			// DONE(student): Compute the new `forward` and `up` vectors.
			// Don't forget to normalize the vectors! Use `glm::rotate()`.

			forward =
				glm::normalize(
					glm::vec3(
						glm::rotate(glm::mat4(1), angle, right) * glm::vec4(forward, 1)));
		...
		{
			// DONE(student): Compute the new `forward`, `up` and `right`
			...
			forward =
				glm::normalize(
					glm::vec3(
						glm::rotate(glm::mat4(1), angle, glm::vec3(0, 1, 0)) * glm::vec4(forward, 1)));

			right =
				glm::normalize(
					glm::vec3(
						glm::rotate(glm::mat4(1), angle, glm::vec3(0, 1, 0)) * glm::vec4(right, 1))
				);
		...
		{
			// DONE(student): Compute the new `right` and `up`. This time,
			// `forward` stays the same. Use `glm::rotate()`. Don't forget
			// to normalize the vectors!
			right =
				glm::normalize(
					glm::vec3(
						glm::rotate(glm::mat4(1), angle, forward) * glm::vec4(forward, 1))
				);

			up =
				glm::normalize(
					glm::vec3(
						glm::rotate(glm::mat4(1), angle, forward) * glm::vec4(up, 1))
				);

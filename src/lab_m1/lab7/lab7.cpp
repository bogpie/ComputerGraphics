...
void Lab7::Init()
{
    ...
    // Create a shader program for drawing face polygon with the color of the normal
    {
        ...
        //shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M1, "lab7", "shaders", "Text.FS.glsl"), GL_FRAGMENT_SHADER);
        //shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M1, "lab7", "shaders", "Text.VS.glsl"), GL_VERTEX_SHADER);
...
void Lab7::Update(float deltaTimeSeconds)
{
    ...
    //gfxc::TextRenderer::TextRenderer textRenderer = gfxc::TextRenderer("test", 1, 1);
...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    // DONE(student): Set light position uniform
    GLint lightPostitionLocation = glGetUniformLocation(shader->program, "light_position");
    glUniform3fv(lightPostitionLocation, 1, glm::value_ptr(lightPosition));


    glm::vec3 eyePosition = GetSceneCamera()->m_transform->GetWorldPosition();
    // DONE(student): Set eye position (camera position) uniform
    ...
    // DONE(student): Set material property uniforms (shininess, kd, ks, object color)
    // shininess
    GLint shininessLocation = glGetUniformLocation(shader->program, "material_shininess");
    glUniform1i(shininessLocation, materialShininess);

    // kd (diffuse)
    GLint kdLocation = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(kdLocation, materialKd);

    // ks (specular)
    GLint ksLocation = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(ksLocation, materialKs);

    // color
    ksLocation = glGetUniformLocation(shader->program, "object_color");
    glUniform3fv(ksLocation, 1, glm::value_ptr(color));

...
void Lab9::Init()
{
	...
	// Create a simple quad
	{
		...
		{
			// DONE(student): Complete texture coordinates for the square
			glm::vec2(0.0f, 0.0f),
			glm::vec2(1.0f, 0.0f),
			glm::vec2(1.0f, 1.0f),
			glm::vec2(0.0f, 1.0f)
...
{
	// DONE(student): Choose an object and add a second texture to it.
	...
	// Why does this happen? How can you solve it?
	{
		...
		modelMatrix = glm::translate(modelMatrix, glm::vec3(5, 1, -3));
...
void Lab9::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	// DONE(student): Set any other shader uniforms that you need
	int isMixedLocation = glGetUniformLocation(shader->program, "isMixed");
	glUniform1i(isMixedLocation, (int) isMixed);
	
	// Rotate when mesh is an Earth, otherwise objects stay in place (and we don't make modifications on time)
...
Texture2D* Lab9::CreateRandomTexture(unsigned int width, unsigned int height)
{
	...
	// DONE(student): Generate random texture data
	for (int index = 0; index < size; ++index)
	{
		data[index] = rand();
	}

	// DONE(student): Generate and bind the new texture ID
	...
	// DONE(student): Set the texture parameters (MIN_FILTER, MAG_FILTER and WRAPPING MODE) using glTexParameteri
	...
	// DONE(student): Generate texture mip-maps

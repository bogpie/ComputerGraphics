...
void Lab2::Init()
{
	...
	// by definition!
	vector<VertexFormat> verticesTetra
	{
	...
	vector<unsigned int> indicesTetra =
	{
		0, 1, 2,
		0, 3, 1,
		0, 2, 3,
	...
	CreateMesh("customTetra", verticesTetra, indicesTetra);


	vector<VertexFormat> verticesColoredTetra
	{
		...
		VertexFormat(glm::vec3(1, 0, 0), glm::vec3(0, 1, 0)), // 3
		VertexFormat(glm::vec3(0, 0, 0), glm::vec3(1, 0, 0)), // 4
		VertexFormat(glm::vec3(0, 0, 1), glm::vec3(1, 0, 0)), // 5
		VertexFormat(glm::vec3(0, 1, 0), glm::vec3(1, 0, 0)), // 6
		VertexFormat(glm::vec3(1, 0, 0), glm::vec3(1, 0, 0)), // 7
		VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0, 0, 1)), // 8
		VertexFormat(glm::vec3(0, 0, 1), glm::vec3(0, 0, 1)), // 9
		VertexFormat(glm::vec3(0, 1, 0), glm::vec3(0, 0, 1)), // 10
		VertexFormat(glm::vec3(1, 0, 0), glm::vec3(0, 0, 1)), // 11
		VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0, 1, 1)), // 12
		VertexFormat(glm::vec3(0, 0, 1), glm::vec3(0, 1, 1)), // 13
		VertexFormat(glm::vec3(0, 1, 0), glm::vec3(0, 1, 1)), // 14
		VertexFormat(glm::vec3(1, 0, 0), glm::vec3(0, 1, 1)), // 15

	};


	vector<unsigned int> indicesColoredTetra =
	{
		0, 1, 2,
		4, 7, 5,
		8, 10, 11,
	...
	CreateMesh("customColoredTetra", verticesColoredTetra, indicesColoredTetra);
	...
	CreateMesh("customSquare", verticesSquare, indicesSquare);


	int noTriangles = 150, radius = 1;
	...
	for (int idTriangle = 0; idTriangle < noTriangles; ++idTriangle)
	{
		...
		verticesCircle.push_back(VertexFormat(glm::vec3(radius * cos(angle), 0, radius * sin(angle)), glm::vec3(1, 0, 0)));
		...
		indicesCircle.push_back(secondIndice);
		indicesCircle.push_back(firstIndice);
	}
	CreateMesh("customCircle", verticesCircle, indicesCircle);
}
...
void Lab2::Update(float deltaTimeSeconds)
{
	...
	cullFaceToggle == true ? glCullFace(GL_FRONT) : glCullFace(GL_BACK);
	...
	RenderMesh(meshes["cube_A"], shaders["VertexNormal"], glm::vec3(-2.5f, 0.5f, 0), glm::vec3(0.5f));


	// TODO(student): Draw the mesh that was created with `CreateMesh()`
	// TODO(student): Draw the tetrahedron
	RenderMesh(meshes["customTetra"], shaders["VertexColor"], glm::vec3(-3.5f, 0.5f, 0), glm::vec3(0.5f));
	RenderMesh(meshes["customColoredTetra"], shaders["VertexColor"], glm::vec3(-4.5f, 0.5f, 0), glm::vec3(0.5f));

	// TODO(student): Draw the square
	RenderMesh(meshes["customSquare"], shaders["VertexColor"], glm::vec3(-5.5f, 0.5f, 0), glm::vec3(0.5f));

	// Bonus: Draw the circle
	RenderMesh(meshes["customCircle"], shaders["VertexColor"], glm::vec3(-6.5f, 0.5f, 0), glm::vec3(0.5f));
